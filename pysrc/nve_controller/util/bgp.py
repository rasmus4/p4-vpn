...
from scapy.fields import (Field, BitField, BitEnumField, XBitField, ByteField,
                          ...
                          MultiEnumField, NBytesField, MACField, ThreeBytesField)
        ...
        if pkt.afi == 2:
            if pkt.safi == 1:
                ...
                while remain:
                    ...
                    lst.append(prefix)
        elif pkt.afi == 25 and pkt.safi == 70:
...
RT_ETHERNET_AD_ROUTE = 1
RT_MAC_IP_ROUTE = 2
RT_INCLUSIVE_MCAST_ROUTE = 3
RT_ETHERNET_SEGMENT_ROUTE = 4

class RTSpecific(Packet):
    pass

class RTSPecificField(PacketField):
    def m2i(self, pkt, m):
        ret = None
        route_type = pkt.route_type

        if route_type == RT_ETHERNET_AD_ROUTE:
            return RTEthernetAutoDiscovery(m)
        elif route_type == RT_MAC_IP_ROUTE:
            return RTMacIPAdvertisementRoute(m)
        elif route_type == RT_INCLUSIVE_MCAST_ROUTE:
            return ret # TODO not implemented
        elif route_type == RT_ETHERNET_SEGMENT_ROUTE:
            return ret # TODO not implemented

        return ret

class RouteDistinguisherValueType0(Packet):
    fields_desc = [
        ShortField("asn", default=0),
        NBytesField("assigned_number", default=0, sz=4)
    ]

class RouteDistinguisherValueType1(Packet):
    fields_desc = [
        IPField("ip_address", default="0.0.0.0"),
        ShortField("assigned_number", default=0)
    ]

class RouteDistinguisherValueType2(Packet):
    fields_desc = [
        NBytesField("asn", default=0, sz=4),
        ShortField("assigned_number", default=0)
    ]

class RouteDistinguisherField(PacketField):
    def m2i(self, pkt, m):
        return self.cls(m[0:8])

    def getfield(self, pkt, s):
        return s[8:], self.m2i(pkt, s)

class RouteDistinguisher(Packet):
    fields_desc = [
        ShortField("type", default=1),
        ConditionalField(
            PacketField("value", default=RouteDistinguisherValueType0(), cls=RouteDistinguisherValueType0),
            lambda p: p.type == 0
        ),
        ConditionalField(
            PacketField("value", default=RouteDistinguisherValueType1(), cls=RouteDistinguisherValueType1),
            lambda p: p.type == 1
        ),
        ConditionalField(
            PacketField("value", default=RouteDistinguisherValueType2(), cls=RouteDistinguisherValueType2),
            lambda p: p.type == 2
        )
    ]

class RTMacIPAdvertisementRoute(RTSpecific):
    fields_desc = [
        RouteDistinguisherField("route_distinguisher", default=RouteDistinguisher(), cls=RouteDistinguisher),
        NBytesField("ethernet_segment_id", default=0, sz=10), # ESI 0: Single-homed site
        NBytesField("ethernet_tag_id", default=0, sz=4), # 0 for VLAN-Based Service Interface (6.1 in RFC 7432), i.e. a single broadcast domain per EVPN instance
        ByteField("mac_address_length", default=48),
        MACField("mac_address", default=0),
        ByteField("ip_address_length", default=0), # length of 32 can be used to determine that IPv4 is used, see 9.2.1 in RFC 7432
        ConditionalField(
            IPField("ip_address", default="0.0.0.0"),
            lambda p: p.ip_address_length == 32
        ),
        BitField("mpls_label1", default=0, size=20),
        BitField("mpls_qos1", default=0, size=3),
        BitField("mpls_bos1", default=1, size=1)
    ]

class RTEthernetAutoDiscovery(RTSpecific):
    fields_desc = [
        RouteDistinguisherField("route_distinguisher", default=RouteDistinguisher(), cls=RouteDistinguisher),
        NBytesField("ethernet_segment_id", default=0, sz=10),
        NBytesField("ethernet_tag_id", default=0, sz=4),
        BitField("mpls_label", default=0, size=20),
        BitField("mpls_qos", default=0, size=3),
        BitField("mpls_bos", default=1, size=1)
    ]

class EVPNNLRI(Packet):
    fields_desc = [
        ByteField("route_type", default=0),
        ByteField("length", default=0), # 25 for RT_ETHERNET_AD, 37 for RT_MAC_IP
        RTSPecificField("route_type_specific", None, cls=RTSpecific)
    ]
