...
import grpc
from .proto import p4runtime_pb2
from .proto import p4runtime_pb2_grpc
from .proto import p4config_pb2
...
class SwitchConnection(object):
    ...
    def WriteTableEntry(self, table_entry, update_entry=False, delete_entry=False, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
        update = request.updates.add()
        if table_entry.is_default_action or update_entry:
    ...
    def WriteDigestEntry(self, digest_id, dry_run=False, max_timeout_ns=0, max_list_size=1):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
        update = request.updates.add()
        ...
        update.entity.digest_entry.config.ack_timeout_ns = 0
        if dry_run:
            print("P4Runtime Enable Digest:", request)
        else:
            self.client_stub.Write(request)
