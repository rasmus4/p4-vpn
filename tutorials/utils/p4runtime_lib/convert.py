...
def decodeMac(encoded_mac_addr):
    return ':'.join(encoded_mac_addr[i:i+1].hex() for i in range(len(encoded_mac_addr)))
...
def encode(x, bitwidth):
    ...
    if type(x) == str:
        ...
        if len(x) != math.ceil(bitwidth / 8):
            raise Exception("Length of bytes != ceil(bitwidth/8)")
        return x
    else:
